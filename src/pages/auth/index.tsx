  ...
  return (
    <AuthShell>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div>
          ...
          <input type="email" required autoComplete="email" className="input"
            placeholder="you@yourpractice.co.uk" value={email} onChange={e => setEmail(e.target.value)} />
        </div>
        <div>
      ...
      <p className="text-center text-sm mt-6" style={{ color: 'var(--text-muted)' }}>
        No account? <Link to="/signup" className="text-brand-600 font-semibold hover:underline">Start free trial</Link>
      </p>
    </AuthShell>
  )
}
...
export function Signup() {
  ...
  const [password, setPassword]   = useState('')
  const [practiceType, setPracticeType] = useState('NHS & Private')
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Create practice
      const { data: prac, error: pracError } = await db
        .from('practices')
        .insert({
          name: practiceName,
          type: practiceType,
          ...
          <form onSubmit={handleStep1} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">First Name</label>
                <input className="input" placeholder="Raj" required value={firstName} onChange={e => setFirstName(e.target.value)} /></div>
              <div><label className="label">Last Name</label>
                <input className="input" placeholder="Patel" required value={lastName} onChange={e => setLastName(e.target.value)} /></div>
            </div>
            <div><label className="label">Work Email</label>
              <input type="email" className="input" placeholder="you@yourpractice.co.uk" required value={email} onChange={e => setEmail(e.target.value)} /></div>
            <div><label className="label">Practice Name</label>
              <input className="input" placeholder="ClearView Opticians" required value={practiceName} onChange={e => setPracticeName(e.target.value)} /></div>
            <div><label className="label">Password</label>
              <input type="password" className="input" placeholder="At least 8 characters" required value={password} onChange={e => setPassword(e.target.value)} /></div>
        ...
        <>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div><label className="label">Practice Type</label>
              <select className="input" value={practiceType} onChange={e => setPracticeType(e.target.value)}>
                <option>NHS & Private</option><option>Independent</option>
                <option>Private Only</option><option>Domiciliary</option>
              </select></div>
            <div><label className="label">City / Town</label>
              <input className="input" placeholder="e.g. Leeds, West Yorkshire" required value={city} onChange={e => setCity(e.target.value)} /></div>
            <button type="submit" disabled={loading}
          ...
          <button type="button" onClick={() => setStep(1)}
            className="w-full text-center text-sm mt-4" style={{ color: 'var(--text-muted)' }}>← Back</button>
        </>
      )}
    </AuthShell>
  )
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="label">Email</label>
            <input type="email" className="input" placeholder="you@yourpractice.co.uk"
              required value={email} onChange={e => setEmail(e.target.value)} /></div>
          <button type="submit" disabled={loading}
